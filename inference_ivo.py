...
def inference_multi_modality_detector(model: nn.Module,
    ...
    for index, pcd in enumerate(pcds):
        # Reduziere pcd auf den reinen Dateinamen
        pcd_filename = os.path.basename(pcd)

        # Suche nach der passenden Annotation in data_list
        data_info = next((d for d in data_list if os.path.basename(d['lidar_points']['lidar_path']) == pcd_filename), None)

        # Wähle je nach cam_type den richtigen Bildpfad
        if cam_type != 'all':
            img = imgs[index]
        else:
            img = imgs  # imgs muss hier ein Verzeichnispfad sein
        ...
        if cam_type != 'all':
            ...
            if box_mode_3d == Box3DMode.LIDAR:
                if 'lidar2img' in data_info['images'][cam_type]:
                    data_['lidar2img'] = np.array(data_info['images'][cam_type]['lidar2img'])
            ...
            for _, img_info in data_info['images'].items():
                if '__CAM_FRONT__' in img_info['img_path']:
                    img_info['img_path'] = osp.join(img, 'CAM_FRONT', img_info['img_path'])
                if '__CAM_FRONT_RIGHT__' in img_info['img_path']:
                    img_info['img_path'] = osp.join(img, 'CAM_FRONT_RIGHT', img_info['img_path'])
                if '__CAM_FRONT_LEFT__' in img_info['img_path']:
                    img_info['img_path'] = osp.join(img, 'CAM_FRONT_LEFT', img_info['img_path'])
                if '__CAM_BACK__' in img_info['img_path']:
                    img_info['img_path'] = osp.join(img, 'CAM_BACK', img_info['img_path'])
                if '__CAM_BACK_LEFT__' in img_info['img_path']:
                    img_info['img_path'] = osp.join(img, 'CAM_BACK_LEFT', img_info['img_path'])
                if '__CAM_BACK_RIGHT__' in img_info['img_path']:
                    img_info['img_path'] = osp.join(img, 'CAM_BACK_RIGHT', img_info['img_path'])
...
def inference_mono_3d_detector(model: nn.Module,
    ...
    Args:
        ...
        cam_type (str): Image of Camera chose to infer.
            ...
            and for nuscenes dataset, it should be 'CAM_FRONT'. Defaults to 'CAM_FRONT'.
...
def inference_segmentor(model: nn.Module, pcds: PointsType):
    ...
    for pipeline in test_pipeline:
        if pipeline['type'] != 'LoadAnnotations3D' and pipeline['type'] != 'PointSegClassMapping':
